...
```html
<div class="csp-slider" aria-label="...">         <!-- root: any element -->
  <div data-slider-track>                          <!-- exactly one, required -->
    <div data-slider-slide>...</div>                <!-- one per slide -->
...
- `root` is passed to `createSlider` directly. It gets `role="region"`,
  ...
  provide your own label describing the carousel's *content*, e.g.
...
| `settle` | `{ index, previousIndex, source: 'scroll' }` | The scroll position has settled (native scroll-snap or a programmatic scroll finished), after any loop boundary correction. Use this instead of `change` if you need to know the *visual* transition is done, not just that a navigation was requested. |
...
| `destroy` | `undefined` | Fired synchronously inside `destroy()`, before listeners are cleared — this is the *last* event any listener will receive. |
...
@media (min-width: 48rem) {
  .my-gallery { --slider-slide-size: calc((100% - 2 * var(--slider-gap)) / 3); }
}
```
...
`--slider-duration` custom property is provided for *your own* transitions
